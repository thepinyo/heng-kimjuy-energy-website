...
function renderReviews(c) {
  ...
  grid.innerHTML = items.map(function (r) {
    ...
    // Avatar: use photo if provided, otherwise show initial letter
    ...
    return '<div class="bg-gray-50 rounded-2xl p-6 hover:shadow-lg transition-shadow flex flex-col">'
      ...
      + photoHtml
      + '<div class="mt-3"><span class="inline-block bg-primary/10 text-primary text-xs font-medium px-2.5 py-1 rounded-full">' + r.service + '</span></div>'
      + '</div>';
  }).join('');
